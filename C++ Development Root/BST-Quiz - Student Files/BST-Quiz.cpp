...
**
** Using only the techniques discussed in class, implement the Binary Search Tree code
...
struct BST
{
  ...
  **       o)  Clearly identify the Base Case, Visit, and Recurse steps with comments
  **           Hint:  Write comments for each step before writing code
  ...
  *****************************************************************************/
  ...
  void print( Node const * current, std::size_t & count )  // in-order traversal
  {
    // Base case
  ...
  void display( Node const * node, std::size_t & count )
  {
    auto && [key, value] = node->_pair;
    std::println ( "{:3}:  {{{}, {}}}", ++count, key, value );
  }
...
int main()
{
  ...
  std::print( "Test Case 5:\n" );

  BST<char, int> anotherTree;
  anotherTree.find ('A');
  anotherTree.erase('B');
  std::print( "myTree._size:       {}\n"
              "anotherTree._size:  {}\n",
              myTree     ._size,
              anotherTree._size );
  ...
  std::print( "Node 'B' found: {}\n"
              "Node 'C' found: {}\n"
              "Node 'D' found: {}\n",
              (n3 == n6 && n6 == anotherTree.find('B')),
              (n1 == n4 && n4 == anotherTree.find('C')),
              (n2 == n5 && n5 == anotherTree.find('D')) );
