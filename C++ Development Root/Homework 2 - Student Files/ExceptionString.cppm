...
export namespace CSUF::CPSC131
{
  ...
  inline std::string exceptionString( const std::string_view message, const std::source_location location = std::source_location::current(), [[maybe_unused]] const unsigned short skip = 1 )
  {
    return std::format( "{}\n detected in function \"{}\"\n"
                        ...
                        location.function_name(), location.line(), location.file_name(),
                          ...
                          #ifdef NDEBUG
                            std::stacktrace::current(0)                                 // don't skip any stacktrace entries in Release builds
                          #else
                            std::stacktrace::current( skip )                            // the number of stacktrace entries to skip in Debug builds
                          #endif
                        #else
