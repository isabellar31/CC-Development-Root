...
/***********************************************************************************************************************************
**  Module Regression.GroceryListTests
**
***********************************************************************************************************************************/
export module Regression.GroceryListTests;
import std;
...
/***********************************************************************************************************************************
**  Module Regression.GroceryListTests Implementation
...
{
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // Namespace scoped objects get constructed BEFORE main(...) begins to execute.  It's this construction process that launches all
  // the regression tests.
  //
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  static GroceryListTests run_grocery_list_tests;




  void GroceryListTests::test()
  {
...

    {
      ...
      try
      {
        for( unsigned i = 0; i < 100; ++i ) list.insert( GroceryItem{ std::format("GroceryItem-{}", i ) } );
  ...
  }  // GroceryListTests::test()




  void GroceryListTests::deepArrayInterrogationTest()
  {
    ...
    // depending on GroceryList.cppm including what I need here.  By creating a struct that mirrors the attribute layout of the
    ...
    // Convert the array to string.  The STL does not define an insertion (i.e., write) operator for std::array, which is needed if
    // an std::array is passed to Regression::CheckResults::is_equal.  So instead of passing standard arrays, let's convert the
    // array to a string and pass the string to Regression::CheckResults::is_equal.  Perhaps a better approach would be to create
    // insertion operator templates in Regression::CheckResults, but this gets the job done with less ripple effect, for now ...
    //
    // Update:  The STL now provides a formatter for std::arrays which has the effect of converting the array to a string.  I could
    // use that here, but I like my formatting better for this situation (and I don't want to specialize the standard formatter)
    auto to_string = []( const auto & container ) -> std::string
    {
      std::string str{ '\n' };

      for( std::size_t index = 0; index < container.size(); ++index )  str += std::format( "{:3}:  {}\n", index, container[index] );
      return str;
    };
    ...
    auto verify_arrays = [&]( char const * const message, std::size_t id )
    {
      ...
      if( expected_array != actual_array )
      {
        ...
        affirm.is_equal( std::format( "Array management deep interrogation (array) #{} :  {}", id, message), to_string( expected_array ), to_string( actual_array ) );
      }

      if( actual_size != expected_size )
      {
        affirm.is_equal( std::format("Array management deep interrogation (size) #{} :  {}", id , message), expected_size, actual_size );
      }
    };
    ...
    {
      GroceryItem gItem{ std::format("item #{}", expected_size), "", "", 9876543.21 };        // something that will be recognizable after a move.  Strings will be cleared but not the price
    ...
    { // Insert at the bottom with 2 empty slots available
      GroceryItem gItem{ std::format("item #{}", __LINE__ ), "", "", 9876543.21 };
    ...
    { //  Moving too much when erasing?
      ...
      {
        actual_array[i] = expected_array[i] = { std::format("item #{}.{}", __LINE__, i), "", "", 9876543.21 };
      }
  ...
  }    // GroceryListTests::deepArrayInterrogationTest()




  GroceryListTests::GroceryListTests()
  {
